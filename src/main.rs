...
async fn main() {
    ...
    let store_path = std::env::args().nth(1).unwrap_or_else(|| ".snerdata/tasks/tasks.log".to_string());
    ...
    while let Ok(Some(line)) = reader.next_line().await {
        ...
        match msg_res {
            Ok(IncomingMessage::Register { task_type }) => {
                ...
                q_clone.register_task_handler(&task_type, move |task: RetryableTask| {
                ...
                queue.register_max_retry_handler(&task_type, move |task: RetryableTask| {
                ...
                println!("{}", serde_json::to_string(&OutgoingMessage::Ack { 
                    message: format!("Registered handler for {}", task_type) 
                }).unwrap());
            }

            Ok(IncomingMessage::Enqueue { task_id, task_type, task_data, max_retries, retry_after_hours }) => {
                let t = RetryableTask::new(task_id, task_type, task_data, max_retries, retry_after_hours);
                if let Err(e) = queue.enqueue(t) {
                    println!("{}", serde_json::to_string(&OutgoingMessage::Error { 
            ...
            Ok(IncomingMessage::Result { task_id, status, error_msg }) => {
                if let Some(tx) = pending_executions.write().await.remove(&task_id) {
                    ...
                    println!("{}", serde_json::to_string(&OutgoingMessage::Error { 
                        message: format!("Received result for unknown/expired task_id {}", task_id) 
                    }).unwrap());
                }
            }
